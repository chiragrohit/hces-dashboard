...
lists into one machine-readable file: hces-code/parquet-code-map.json.
...
Run: python build_parquet_code_map.py  (from the project root)
"""
import csv
import json
import os
...
# Dashboard chart group -> code_map column. The frontend's code->label maps
# (web_dashboard/js/code-maps.generated.js) are generated from these so the
# hand-written copies never drift from the questionnaire again.
CHART_MAPS = {
    "marital": "Marital_Status",
    "relation": "Relation_to_Head",
    "social": "Social_Group_of_HH_Head",
    "lighting": "Energy_Source_Lighting",
    "cooking": "Energy_Source_Cooking",
    "dwelling": "Type_of_Dwelling_Unit",
    "land": "Land_Ownership",
    "religion": "Religion_of_HH_Head",
    "hhtype": "Household_Type",
    "ration": "Ration_Card_Type",
    "foodsrc": "Source",
}

# Non-food / durable charts read specific consumption item codes; map each
# chart key to (parquet table, [item codes]) using the item names from the
# same code map so labels never drift from the questionnaire.
CHART_ITEMS = {
    "clothing": ("consumption_13", ["379", "389", "399"]),
    "services": ("consumption_9_10_11", ["409", "419", "429", "499", "519", "539"]),
    "fuel": ("consumption_8_1", ["331", "332", "333", "334", "335", "336", "337", "338", "340", "341", "342", "343", "346", "096"]),
    "tobacco": ("consumption_12", ["319", "329"]),
    "durables": ("consumption_14", ["580", "623", "560", "590", "588", "585", "601", "581", "602"]),
}

JS_OUT = os.path.join(BASE, "web_dashboard", "js", "code-maps.generated.js")


def write_chart_maps(doc):
    """Write the dashboard's code->label maps from the code map."""
    c = {}
    for key, col in CHART_MAPS.items():
        m = doc["columns"][col]["map"]
        c[key] = {str(k): v for k, v in m.items()}
    for key, (table, codes) in CHART_ITEMS.items():
        items = doc["tables"][table]["items"]
        c[key] = {str(code): items.get(str(code), "Item " + str(code)) for code in codes}
    js = ("/* AUTO-GENERATED by build_parquet_code_map.py from parquet-code-map.json - do not edit. */\n"
          "export const C = " + json.dumps(c, ensure_ascii=False, indent=2) + ";\n")
    with open(JS_OUT, "w", encoding="utf-8") as f:
        f.write(js)
    print(f"Chart maps written: {len(c)} groups -> web_dashboard/js/code-maps.generated.js")
...
def parse_annotation():
    ...
    ocr-questionnaire-annotation.json already follows the extraction schema: sections
...
def main():
    ...
    if os.path.exists(OCR_JSON):
        ...
        src = "Mistral-OCR markdown (hces-code/HCES-ocr-mistral/markdown.md)"
        print(f"Source: {src}")
        sections = parse_ocr_sections(ocr_markdown())
        section_items = section_items_from_ocr(sections)
    else:
        src = "pypdf text extraction of the questionnaire PDF (LAST-RESORT FALLBACK)"
        print(f"WARNING: ocr-questionnaire-annotation.json is missing - using {src}")
        print("WARNING: OCR output was deleted; re-run the Mistral OCR to restore the primary source")
        text = pdf_text()
    ...
    doc = {
        "_meta": {
            ...
            "generated_by": "build_parquet_code_map.py",
            "actual_source": src,
    ...
    write_chart_maps(doc)

    total_items = sum(len(t["items"]) for t in doc["tables"].values())
    print(f"\nparquet-code-map.json written: {len(doc['columns'])} column maps, "
